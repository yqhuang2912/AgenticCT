...
SAVE_DIR = "/data/hyq/codes/AgenticCT/intermediates"

def save_tensor_to_npy(tensor, filename: str):
    """Save a PyTorch tensor as a .npy file."""
    array = tensor.cpu().numpy()
    np.save(os.path.join(SAVE_DIR, filename), array)

def load_npy_to_tensor(filename: str) -> torch.Tensor:
    """Load a .npy file as a PyTorch tensor."""
...
def visualize_tensor(tensor, filename, vmin=None, vmax=None):
...
def convert_to_image_range(image: np.ndarray, vmin: float, vmax: float) -> np.ndarray:
    """Convert image to [0, 255] range based on given vmin and vmax."""
    image = np.clip(image, vmin, vmax)
    image = (image - vmin) / (vmax - vmin)  # Normalize to [0, 1]
    image = (image * 255.0).astype(np.uint8)  # Scale to [0, 255]
    return image